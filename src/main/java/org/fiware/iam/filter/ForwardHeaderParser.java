 ...
 *
 * <p>This parser handles both the standard RFC 7239 `Forwarded` header and configurable legacy
 * `X-Forwarded-*` headers. It extracts key information such as:
 *
 * <ul>
 *   <li>Original client IP address (`for` / X-Forwarded-For)
 *   <li>Original protocol (`proto` / X-Forwarded-Proto)
 *   <li>Host and optional port (`host` / X-Forwarded-Host and X-Forwarded-Port)
 *   <li>Proxy information (`by`)
 *   <li>Path prefix added by upstream proxies (`X-Forwarded-Prefix`)
 * </ul>
 */
@Singleton
public class ForwardHeaderParser {
  ...
  public ForwardHeaderParser(
      ForwardedForConfig config,
   ...
   *
   * <p>This method reads both the standard `Forwarded` header (RFC 7239) and any legacy
   * `X-Forwarded-*` headers as configured in {@link ForwardedForConfig}. It populates a {@link
   * ForwardedInfo} object with the extracted details: client IP (`for`), protocol (`proto`), host,
   * port, proxy (`by`), and path prefix. `Forwarded` header values take precedence over legacy
   * headers when both are present.
   *
   * <p>The returned {@link ForwardedInfo} object can be used by downstream filters or controllers
   * to reconstruct the original request URL or handle redirects correctly when behind reverse
   * proxies.
   *
  ...
  public ForwardedInfo parse(HttpRequest<?> request) {
    ...
    ForwardedInfo forwardedInfo =
        new ForwardedInfo(null, defaultServerProtocol, defaultHost, serverPort, null, "");
    ...
    if (port == 0) {
      forwardedInfo.setForwardedPort(
          HTTPS_PROTOCOL.equalsIgnoreCase(proto) ? DEFAULT_HTTPS_PORT : DEFAULT_HTTP_PORT);
    } else if (port == -1) {
