 ...
 *
 * <p>This filter reads the RFC 7239 Forwarded header as well as legacy X-Forwarded-* headers (such
 * as X-Forwarded-For, X-Forwarded-Proto, X-Forwarded-Host, X-Forwarded-Prefix) from each request.
 * It parses these headers to determine the original client IP, protocol, host, port, and any path
 * prefix applied by upstream proxies or gateways.
 *
 * <p>The filter then computes the full original request URL and stores it, along with the parsed
 * forwarding details, as attributes in the request:
 *
 * <ul>
 *   <li>{@link #REQ_ATTR}: the reconstructed request URI
 *   <li>{@link #FORWARD_INFO_ATTR}: the {@link ForwardedInfo} object containing all parsed
 *       forwarding information
 * </ul>
 *
 * <p>Downstream controllers and filters can use these attributes to correctly generate absolute
 * URLs, redirects, or logs that reflect the original client-facing request even when behind reverse
 * proxies.
 */
...
public class ForwardedForFilter implements HttpServerFilter, Ordered {
  ...
  public Publisher<MutableHttpResponse<?>> doFilter(
    ...
    HttpRequest<?> modifiedRequest =
        request.setAttribute(REQ_ATTR, reqUrl).setAttribute(FORWARD_INFO_ATTR, forwardedInfo);
  ...
  private URI getReqUrl(ForwardedInfo forwardedInfo) {
    ...
    if (!(HTTP_PROTOCOL.equalsIgnoreCase(protocol) && port == ForwardHeaderParser.DEFAULT_HTTP_PORT)
        && !(HTTPS_PROTOCOL.equalsIgnoreCase(protocol)
            && port == ForwardHeaderParser.DEFAULT_HTTPS_PORT)) {
      portToUse = port;
    }
